export async function initPage() {
  try {
    await loading();
    const idMateri = sessionStorage.getItem('selected_materi_id');
    if (!idMateri) {
...
function renderMateriDetail(materi) {
  ...
  document.getElementById('author').textContent = materi.authorName;
  ...
  document.getElementById('jurusan').textContent = materi.jurusanName;
...
function getFileInfo(extension) {
  const types = {
    ...
    jpg: { icon: 'fa-file-image', color: 'text-green-500', type: 'Image' },
    jpeg: { icon: 'fa-file-image', color: 'text-green-500', type: 'Image' },
    png: { icon: 'fa-file-image', color: 'text-green-500', type: 'Image' },
    gif: { icon: 'fa-file-image', color: 'text-green-500', type: 'Image' },
    ...
    return `
        <div class="h-96">
          ...
          <div class="text-xs text-gray-500 mt-2 text-center">
            Preview menggunakan Microsoft Office Online Viewer (read-only). Jika kosong, cek akses publik file.
          </div>
        </div>
      `;
    default:
...
window.openFullscreenPreview = function(fileUrl, extension) {
  ...
  if (extension.match(/(jpg|jpeg|png|gif)$/)) {
    previewContent = `<img src="${previewSrc}" class="max-h-full max-w-full object-contain" alt="Fullscreen Preview">`;
  } else {
    previewContent = `<iframe src="${previewSrc}" class="w-full h-full" frameborder="0" allowfullscreen referrerpolicy="no-referrer-when-downgrade"></iframe>`;
  }
  overlay.innerHTML = `
    <div class="flex justify-between items-center mb-4">
      <h3 class="text-xl font-semibold text-white">Preview File</h3>
      <button id="closePreviewBtn" class="text-white hover:text-gray-300 text-2xl">&times;</button>
    </div>
    <div class="flex-1 flex items-center justify-center">
      ${previewContent}
    </div>
    <div class="mt-4 flex justify-center space-x-4">
      <a id="downloadPreviewLink" href="${fileUrl}" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">
        <i class="fas fa-download mr-2"></i>Unduh File
      </a>
      <button id="closePreviewBtn2" class="px-6 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg">Tutup</button>
    </div>
    <div id="officeWarning" class="text-sm text-gray-300 mt-3 text-center" style="display: none;">
      Jika tampilan kosong, kemungkinan file tidak dapat diakses publik atau diblokir oleh host (cek URL publik / Content-Disposition).
    </div>
  `;
  document.body.appendChild(overlay);
  overlay.querySelectorAll('#closePreviewBtn, #closePreviewBtn2').forEach(btn => {
  ...
  if (isOffice) {
    ...
    setTimeout(() => {
      if (!loaded) {
        const warn = document.getElementById('officeWarning');
...
function renderFile(fileUrl) {
  ...
  `;
  loadPreviewer(fileUrl, fileExtension);
}
  ...
  if (container) {
    container.innerHTML = `
      <div class="text-center py-10">
        ...
        <a href="#/materi_list" class="inline-block bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg">
...
function loadPreviewer(fileUrl, extension) {
  if (extension === 'pdf') {
