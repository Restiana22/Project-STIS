// laporan_kegiatan_guru.js - Dengan Server-side Pagination
...
export async function initPage() {
    const config = {
        ...
        title: 'Laporan Absensi Mengajar',
        icon: 'activity',
        enableStatistics: false,
        emptyStateTitle: 'Belum ada data kegiatan',
        emptyStateMessage: 'Data kegiatan akan muncul setelah guru melaporkan aktivitas.',
        emptyStateIcon: 'calendar',
        pageSize: 50,
        
        filterableColumns: (() => {
            const columns = [
                { key: 'guru_name', label: 'Nama Guru', type: 'text' },
            ...
            if (user.role === 'admin' || user.role === 'bk') {
                columns.unshift({ key: 'guru_no', label: 'No. Guru', type: 'text' });
            }
        ...
        dataFetcher: async (page, pageSize, filters) => {
            ...
            const processedData = data.map(kegiatan => {
                ...
                return {
                    id: kegiatan.id,
                    guru_name: userData?.name || 'Unknown',
                    guru_no: userData?.user_no || '-',
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <h4 class="font-medium text-gray-700 dark:text-gray-300 mb-2">Informasi Guru</h4>
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Nama:</span>
                            <span class="font-medium">${item.guru_name}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">No. Guru:</span>
                            <span class="font-medium">${item.guru_no}</span>
                        </div>
                    </div>
                </div>
